...
1. 创建一个 1 ~ 100 的循环，当数字是奇数时，打印“奇数”，否则打印“偶数”。

   提示：使用运行器提供的 `print` 函数来进行“打印”操作。

2. 显示一个九九乘法表，使用 `print` 函数来打印每一行。
...
我们使用 for 语句依次访问了数组中的每一个元素，变量 `i` 表示数组元素的*索引*，如果这个索引值小于数组长度，说明还没有到数组尽头，那么就继续进行处理，否则就停止循环。这样访问数组每一个元素的流程称为*遍历*。