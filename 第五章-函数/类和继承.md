...
首先，`Car` 是一个类，它拥有一个构造函数。当我们像这样：`new Car("any", "any")` 实例化一个类时，我们会接受一些参数，实际上调用了类的构造函数，并在构造函数中完成了完成了构造一个对象所需做的事情。构造函数中会拥有一个 `this` 关键字，它所拥有的属性，所有的实例都会拥有。`this` 在这里相当于创建对象所遵循的“原型”，`new` 关键字的作用则是根据对象的原型创建具体的对象。
...
前面已经说过，类是一种特殊的函数，它也可以拥有自己的属性与方法。所有的类都具有一个 `prototype` 属性，它是所有实例的原型，原型的属性和方法就是实例的属性和方法。在一个类中， `this` 关键字实际上指这个原型。我们可以通过手动为原型的属性赋值，来达到相同的目的。
