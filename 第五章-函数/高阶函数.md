...
接下来就是有办法打破这个问题。我们可以使用可变参数函数执行此操作：
...
如果不是退化的情况，我们需要将我们的解决方案与其他解决方案结合起来。换句话说，我们需要结合`fn`使用`compose(...rest)`。我们怎么做？好吧，考虑一下`compose(a, b)`。我们知道这`compose(b)`是堕落的情况，只是`b`。我们知道`compose(a, b)`就是这样`(c) => a(b(c))`。

因此，让我们替换`compose(b)`为`b`：
...
当然还有其他人。`compose`可以通过迭代实现，或者`.reduce`像这样：
...
但主要行为是相同的：一起组成一系列功能，创建一个新功能。价值是一样的：我们可以编写更小的单一用途函数，并以不同的方式将它们组合在一起。
