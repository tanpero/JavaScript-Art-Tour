...
JavaScript 的数组方法中包括三个高阶函数，以更方便地处理数据。
...
在该示例中，它看起来与您在大多数语言中找到的映射函数完全相同：您向它传递一个函数，并使用一个参数（数组的元素）调用该函数。然而，这不是整个故事。JavaScript `map`实际上使用*三个*参数调用每个函数：元素，数组中元素的索引以及数组本身。
...
如果传入一个只接受一个参数的函数，它就会忽略其他参数。但是有些函数有可选的第二个甚至第三个参数。例如：
...
这不起作用，因为`parseInt`定义为`parseInt(string[, radix])`。它需要一个可选的基数参数。当你打电话`parseInt`时`map`，索引被解释为基数。不好！我们想要的是转换`parseInt`成只接受一个参数的函数。

我们可以写`['1', '2', '3'].map((s) => parseInt(s))`，或者我们可以想出一个装饰师为我们做这个工作：
...
在本节中，我们将接触使用高阶函数实现的一些奇妙技巧。如果初次阅读时无法理解这些示例的含义，可以暂时略过，因为这里运用了一些*魔法*，以供佐餐使用 :-D

**B 组合子**

*B 组合子*是最基本的函数组合应用。例如，我们有两个函数，分别负责将参数加一和乘二（它们分别代表了一些生活中常见的小函数）。如果我们要编写一个“将参数加一然后乘二”的函数，基于“代码复用”的思想，我们或许会这样写：
...
而 B 组合子是一种工具，它可以将两个函数的功能有机组合起来：
...
K 组合子也是基本的函数组合应用之一，它的定义看起来非常简单。

```javascript
const K = (x) => (y) => x;
```

它有一些令人惊讶的应用。当我们需要利用值来做一些事情时，可以用到它。
...
`tap`是从各种 Unix shell 借来的传统名称。它接受一个值，并返回一个总是返回值的函数。如果我们传递一个函数，它就会将值交给函数并执行它。它可以帮助我们用更优雅的方式来处理值与函数的关系。
...
像图书馆[下划线](http://underscorejs.org/)使用的版本`tap`是“uncurried：”
...
如果我们希望它什么都不做，我们可以写任何一个`tap('espresso')()`或者`tap('espresso', null)`

`tap`可以做的不仅仅是充当调试辅助工具。它在处理对象和实例方法时也很有用。
