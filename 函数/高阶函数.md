...
filter 一词在英语中意为“过滤”，而 JavaScript 的数组所拥有的 `filter` 方法，便是根据指定规则过滤出所需要的数据。
...
我们发现，两次不同任务所需的代码几乎是重复的，只是筛选数据的条件有所改动。利用数组的 `filter` 方法，筛选的条件可以单独抽取出来，直接对每个数据进行判断。
...
对于批量处理数据这类工作而言，`map` 和 `filter` 显得很酷。JavaScript 的数组还有个更酷的方法叫 `reduce`。
...
`reduce` 所做的事情似乎不太好理解。其实，比较两种代码可以发现，`reduce` 在内部定义了一个
